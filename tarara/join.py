...
class ara_join(ara_form):
	...
	header = urwid.AttrWrap(header, 'reversed')
        ...
        iddesc = urwid.Filler(urwid.Text(_("ID's length should be\nbetween 4 and 10 chars")))
        ...
        self.pwedit = urwid.Filler(widget.PasswordEdit(caption=_('Password:'), wrap='clip'))
        pwdesc = urwid.Filler(urwid.Text(_('Minimum password length\nis 4 characters')))
        self.pwcolumn = widget.EasyColumn(self.pwedit, pwdesc)

        self.confirmedit = urwid.Filler(widget.PasswordEdit(caption=_('Confirm\nPassword:'), wrap='clip'))
        confirmdesc = urwid.Filler(urwid.Text(_('Type password here\nonce again')))
        self.confirmcolumn = widget.EasyColumn(self.confirmedit, confirmdesc)

        self.emailedit = urwid.Filler(urwid.Edit(caption=_('E-Mail:'), wrap='clip'))
        emaildesc = urwid.Filler(urwid.Text(_("We'll send confirmation\nmail to this address")))
	...
	self.langcolumn = widget.EasyColumn(self.lang, langdesc)
        ...
        self.joinbutton = urwid.Filler(urwid.Button(_('Join'), self.on_button_clicked))
