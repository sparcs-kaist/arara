...
class ara_change_basic_info(ara_form):
    ...
    def on_button_clicked(self, button):
        ...
        if button == self.okbutton.body:
            if self.server.member_manager.is_registered_nickname(self.nickedit.body.get_edit_text()):
                confirm = widget.Dialog("Nickname is in use.\nTry another one.", ["OK"], ('menu', 'bg', 'bgf'), 30, 6, self)
                ...
                if retvalue[0] == True:
                    ...
                    message = retvalue[1]
                confirm = widget.Dialog(message, ["OK"], ('menu', 'bg', 'bgf'), 30, 6, self)
                self.overlay = confirm
                self.parent.run()
                if confirm.b_pressed == "OK":
	...
	self.header = urwid.Filler(urwid.Text("ARA: Change Basic Information", align='center'))
        self.header = urwid.AttrWrap(self.header, 'reversed')
        self.myinfo = self.server.member_manager.get_info(self.session_key)
        self.myinfo = self.myinfo[1]

        idtext = urwid.Filler(urwid.Text("ID: %s" % self.myinfo['username']))
        iddesc = urwid.Filler(urwid.Text("ID and E-Mail couldn't be edited\nDisplayed for your information"))
        idcolumn = widget.EasyColumn(idtext, iddesc)

        emailtext = urwid.Filler(urwid.Text("E-Mail: %s" % self.myinfo['email']))
